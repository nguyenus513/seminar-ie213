...
const { INDEX_STRATEGIES } = require("./index.service");
...
async function runBenchmark({ queryKey, strategyKey = "NO_INDEX", mode = "custom", iterations = 5, limit = 500, saveLog = true }) {
  ...
  const payload = {
    ...
    strategyName: INDEX_STRATEGIES[strategyKey]?.name || strategyKey,
    mode,
...
async function runAllBenchmarks({ iterations = 5, limit = 500, mode = "custom", saveLog = true }) {
  ...
  for (const [queryKey, config] of Object.entries(QUERY_CATALOG)) {
    try {
      results.push(await runBenchmark({ queryKey, strategyKey: config.recommendedStrategy || "NO_INDEX", mode, iterations, limit, saveLog }));
    } catch (error) {
...
async function compareLatest() {
  ...
  for (const queryKey of queryKeys) {
    const baseline = await BenchmarkRun.findOne({ queryKey, $or: [{ mode: "baseline" }, { indexStrategy: "NO_INDEX" }] }).sort({ createdAt: -1 }).lean();
    const optimized = await BenchmarkRun.findOne({ queryKey, $or: [{ mode: "optimized" }, { indexStrategy: QUERY_CATALOG[queryKey].recommendedStrategy }] }).sort({ createdAt: -1 }).lean();
    if (baseline && optimized) {
      ...
      comparisons.push({ queryKey, queryName: QUERY_CATALOG[queryKey].label, baseline, optimized, improvement: { fasterRatio: Number(fasterRatio.toFixed(2)), docsReductionPercent: Number(docsReductionPercent.toFixed(2)) } });
    }
  }
  return comparisons;
}
